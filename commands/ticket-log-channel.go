...
func handleTicketLogChannel(s *discordgo.Session, i *discordgo.InteractionCreate) *discordgo.InteractionResponse {
	...
	if err != nil {
		...
		return components.EmbedResponse(components.ErrorEmbed("Failed to set ticket log channel."), true)
	}
	...
	if err != nil {
		return components.EmbedResponse(components.ErrorEmbed("Failed to send message to ticket log channel."), true)
	}
