...
command.addStringOption((option) =>
    option.setName("options").setDescription("Options for the poll, put up to 8 separated with ||").setRequired(true)
);
    ...
    if (optionsArr?.length) {
        if (optionsArr?.length > 8) {
            return await interaction.reply({
                content: "You can only have up to 8 options in a poll.",
                ephemeral: true
            });
        }
    }
