...
// create the command
const command = new SlashCommandBuilder().setName("log").setDescription("Work with the logging system");
...
async function executeSetChannel(interaction: ChatInputCommandInteraction) {
    ...
    const dbData = await db
        .select()
        .from(logsettings)
        .where(eq(logsettings.guild_id, interaction.guild?.id as string))
        .limit(1)
        .execute();
