...
command.addSubcommand((subcommand) =>
    subcommand
        ...
        .addUserOption((option) =>
            option.setName("user").setDescription("The user you want to add a case to").setRequired(true)
        )
        .addStringOption((option) =>
            option.setName("reason").setDescription("The reason for the case").setRequired(true)
        )
);
command.addSubcommandGroup((group) =>
    group
        ...
        .addSubcommand((subcommand) =>
            subcommand.setName("latest").setDescription("Remove the latest case in your server")
        )
        ...
        .addSubcommand((subcommand) =>
            subcommand
                ...
                .addUserOption((option) =>
                    option.setName("user").setDescription("The user you want to remove all cases for").setRequired(true)
                )
        )
);
command.addSubcommandGroup((group) =>
    group
        ...
        .addSubcommand((subcommand) =>
            subcommand.setName("latest").setDescription("View the latest case in your server")
        )
        .addSubcommand((subcommand) =>
            subcommand
                .setName("user")
                ...
                .addUserOption((option) =>
                    option.setName("user").setDescription("The user you want to view all cases for").setRequired(true)
                )
        )
);
...
async function executeCasesRemoveId(interaction: ChatInputCommandInteraction) {
    ...
    try {
        ...
        await db
            ?.delete(cases)
            .where(and(eq(cases.id, id?.toString() || "0"), eq(cases.guild_id, interaction.guild?.id || "0")))
            .execute();
    } catch (error) {
        logger.error(error);
        const embedData = {
...
async function executeCasesRemoveLatest(interaction: ChatInputCommandInteraction) {
    ...
    const latestCase = await db
        ?.select()
        .from(cases)
        .where(eq(cases.guild_id, interaction.guild?.id || "0"))
    ...
    try {
        await db
            ?.delete(cases)
            .where(and(eq(cases.id, latestCase?.[0].id || "0"), eq(cases.guild_id, interaction.guild?.id || "0")))
            .execute();
    } catch (error) {
        logger.error(error);
        const embedData = {
...
async function executeCasesRemoveUser(interaction: ChatInputCommandInteraction) {
    ...
    try {
        await db
            ?.delete(cases)
            .where(and(eq(cases.user_id, user?.id || "0"), eq(cases.guild_id, interaction.guild?.id || "0")))
            .execute();
    } catch (error) {
        logger.error(error);
        const embedData = {
