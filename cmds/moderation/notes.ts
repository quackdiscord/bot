    ...
    group
        ...
        .addSubcommand((subcommand) =>
            subcommand
                ...
                .addStringOption((option) =>
                    option.setName("id").setDescription("The ID of the note you want to remove").setRequired(true)
                )
        )

        .addSubcommand((subcommand) =>
            subcommand.setName("latest").setDescription("Remove the latest note in your server")
        )
        ...
        .addSubcommand((subcommand) =>
            subcommand
                ...
                .addUserOption((option) =>
                    option.setName("user").setDescription("The user you want to remove all notes for").setRequired(true)
                )
        )
);
command.addSubcommandGroup((group) =>
    group
        ...
        .addSubcommand((subcommand) =>
            subcommand
                ...
                .addStringOption((option) =>
                    option.setName("id").setDescription("The ID of the note you want to view").setRequired(true)
                )
        )
        .addSubcommand((subcommand) =>
            subcommand.setName("latest").setDescription("View the latest note in your server")
        )
        .addSubcommand((subcommand) =>
            subcommand
                .setName("user")
                ...
                .addUserOption((option) =>
                    option.setName("user").setDescription("The user you want to view all notes for").setRequired(true)
                )
        )
);
...
async function executeNoteRemoveId(interaction: ChatInputCommandInteraction) {
    ...
    try {
        await db
            ?.delete(notes)
            .where(and(eq(notes.id, id || "0"), eq(notes.guild_id, interaction.guild?.id || "0")))
            .execute();
    } catch (error) {
        logger.error(error);
        const embedData = {
...
async function executeNoteRemoveLatest(interaction: ChatInputCommandInteraction) {
    ...
    const latestNote = await db
        ?.select()
        .from(notes)
        .where(eq(notes.guild_id, interaction.guild?.id || "0"))
    ...
    try {
        await db
            ?.delete(notes)
            .where(and(eq(notes.id, latestNote?.[0].id || "0"), eq(notes.guild_id, interaction.guild?.id || "0")))
            .execute();
    } catch (error) {
        logger.error(error);
        const embedData = {
...
async function executeNoteRemoveUser(interaction: ChatInputCommandInteraction) {
    ...
    try {
        await db
            ?.delete(notes)
            .where(and(eq(notes.user_id, user?.id || "0"), eq(notes.guild_id, interaction.guild?.id || "0")))
            .execute();
    } catch (error) {
        logger.error(error);
        const embedData = {
...
async function executeNoteViewId(interaction: ChatInputCommandInteraction) {
    ...
    const embedData = {
        description:
            "<:PepoG:1172051306026905620> **Note for** <@" +
            note?.[0].user_id +
            "> \n\nWritten by: <@" +
            note?.[0].moderator_id +
            ">\n```" +
...
async function executeNoteViewUser(interaction: ChatInputCommandInteraction) {
    ...
    const embedData = {
        description:
            "<:PepoG:1172051306026905620> **" + notesData.length + " Notes for** <@" + user?.id + ">\n\n" + notesString,
        color: "DarkButNotBlack",
        timestamp: new Date()
    };
